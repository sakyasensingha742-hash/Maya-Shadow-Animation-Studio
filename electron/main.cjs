...
app.whenReady().then(() => {
  ...
  ipcMain.handle('studio:open-releases', async () => {
    await shell.openExternal('https://github.com/sakyasensingha742-hash/Maya-Shadow-Animation-Studio/releases');
    return { status: 'opened' };
  });
  ipcMain.handle('studio:save-project', async (_event, payload) => {
    const result = await dialog.showSaveDialog(mainWindow, { title: 'Save Maya Shadow Project', defaultPath: `${payload?.name || 'maya-shadow-project'}.maya.json`, filters: [{ name: 'Maya Shadow Project', extensions: ['maya.json'] }, { name: 'JSON', extensions: ['json'] }] });
    if (result.canceled || !result.filePath) return { canceled: true };
    fs.writeFileSync(result.filePath, JSON.stringify(payload, null, 2), 'utf8');
    return { canceled: false, filePath: result.filePath };
  });
  ipcMain.handle('studio:load-project', async () => {
    const result = await dialog.showOpenDialog(mainWindow, { title: 'Open Maya Shadow Project', properties: ['openFile'], filters: [{ name: 'Maya Shadow Project', extensions: ['json'] }] });
