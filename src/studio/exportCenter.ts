...
const presets:Preset[]=[
 ...
 {name:'Custom',platform:'Studio',width:1920,height:1080,ratio:'Custom',note:'Set your own canvas'}
];
function boot(){
 if(document.querySelector('.export-center'))return;
 const root=document.createElement('section');root.className='export-center';
 root.innerHTML=`<div class="ec-card"><div class="ec-head"><div><span>RENDER & EXPORT</span><h2>Export Center</h2><p>Social media, YouTube এবং professional delivery-এর জন্য ready-made canvas preset।</p></div><button class="ec-close" aria-label="Close">×</button></div><div class="ec-presets"><b>SOCIAL MEDIA & PLATFORM PRESETS</b><div class="ec-preset-grid">${presets.map((p,i)=>`<button data-preset="${i}" class="${p.name==='YouTube Landscape'?'active':''}"><strong>${p.name}</strong><small>${p.platform} • ${p.width} × ${p.height} • ${p.ratio} • ${p.note}</small></button>`).join('')}</div></div><div class="ec-grid"><div class="ec-panel"><b>OUTPUT</b><label>Format<select data-format><option value="mp4">MP4 Video</option><option value="webm">WebM Video</option><option value="png-sequence">PNG Sequence</option></select></label><div class="ec-two"><label>Width<input data-width type="number" min="320" step="1" value="1920"></label><label>Height<input data-height type="number" min="240" step="1" value="1080"></label></div><div class="ec-ratio" data-ratio>Aspect Ratio • 16:9</div><label>FPS<select data-fps><option>24</option><option>25</option><option>30</option><option>60</option></select></label><label>Quality<select data-quality><option value="draft">Draft</option><option value="preview" selected>Preview</option><option value="final">Final</option></select></label><label class="ec-check"><input data-transparent type="checkbox"> Transparent background</label></div><div class="ec-panel"><b>FRAME RANGE</b><div class="ec-two"><label>Start<input data-start type="number" min="1" value="1"></label><label>End<input data-end type="number" min="1" value="240"></label></div><div class="ec-summary" data-summary>YouTube Landscape • 1920 × 1080 • 16:9 • 24 FPS • MP4</div><button class="ec-queue">＋ Add to Render Queue</button><button class="ec-save">⇩ Save Render Plan</button><div class="ec-queue-state" data-state>Queue is empty</div></div></div><div class="ec-foot"><span>● Platform presets ready</span><small>Preset নির্বাচন করলে canvas size ও aspect ratio automatically সেট হবে। Custom দিয়ে নিজের resolution ব্যবহার করতে পারবেন।</small></div></div>`;
 ...
 const getRatio=(w:number,h:number)=>{if(!w||!h)return 'Custom';const value=w/h;const known:[[number,number,string]]|Array<[number,number,string]>=[[16,9,'16:9'],[9,16,'9:16'],[4,5,'4:5'],[1,1,'1:1']];return known.find(([a,b])=>Math.abs(value-a/b)<0.005)?.[2]||'Custom'};
