...
export const defaultRetargetProfile:RetargetProfile={
 name:'Mixamo → 2D Character',
 sourceBones:['Hips','Spine','Spine1','Neck','Head','LeftShoulder','LeftArm','LeftForeArm','LeftHand','RightShoulder','RightArm','RightForeArm','RightHand','LeftUpLeg','LeftLeg','LeftFoot','RightUpLeg','RightLeg','RightFoot'],
 targetBones:['root','spine','chest','neck','head','shoulder.L','arm.L','forearm.L','hand.L','shoulder.R','arm.R','forearm.R','hand.R','thigh.L','shin.L','foot.L','thigh.R','shin.R','foot.R'],
 scale:1,mirror:false,rootMotion:true
};

export const motionFormatLabels:Record<MotionFormat,string>={
 'mixamo-fbx':'Mixamo FBX','gltf':'glTF','glb':'GLB','bvh':'BVH Motion Capture','vrm':'VRM','motion-json':'Maya Shadow Motion JSON'
};

export function canRetargetTo2D(format:MotionFormat){return format!=='3d-character' as never}
...
// The UI can accept native motion files and route them through a retarget adapter.
// Actual binary decoding is kept behind adapters so offline decoders can be bundled independently.
