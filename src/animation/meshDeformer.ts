...
export type BoneTransform={rotation:number;originX:number;originY:number};
...
export function createGridMesh(width:number,height:number,gridX=24,gridY=14,bones:RigBone[]=[]):DeformableMesh{
...
export function deformMesh(mesh:DeformableMesh,bones:RigBone[],poses:Record<string,number>={}):MeshVertex[]{
 ...
 return mesh.vertices.map(v=>{
  ...
  for(const [id,w0] of Object.entries(v.weights)){
   ...
   const a=poses[id]??b.rotation;
   const p=rotate(v.x,v.y,b.x,b.y,a-b.rotation);
   x+=p.x*w0;y+=p.y*w0;total+=w0;
  }
  return total?{...v,x:x/total,y:y/total}:v;
 });
}
