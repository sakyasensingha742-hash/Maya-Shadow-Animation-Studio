...
export default function AudioVideoStudioPanel({assets,onImport,onStatus}:Props){
 ...
 const [mode,setMode]=useState<'audio'|'video'>(media.find(a=>a.id===selectedId)?.type==='video'?'video':'audio');
 ...
 const preview=()=>{if(mode==='audio'&&audioRef.current){audioRef.current.volume=Math.min(1,Math.max(0,Math.pow(10,gain/20)));audioRef.current.play();}else videoRef.current?.play();};
 return <div className="av-studio">
  ...
  <div className="av-tabs"><button className={mode==='audio'?'active':''} onClick={()=>setMode('audio')}>🎚 Audio Lab</button><button className={mode==='video'?'active':''} onClick={()=>setMode('video')}>🎬 Video Lab</button><button>🧠 AI Finishing</button><button>📈 Loudness / Scope</button></div>
  <div className="av-layout">
   ...
   <section className="av-editor">
    ...
    {mode==='audio'?<>
      ...
      <div className="video-controls"><label>In <input type="range" min="0" max="100" value={trimStart} onChange={e=>setTrimStart(+e.target.value)}/>{trimStart}%</label><label>Out <input type="range" min="0" max="100" value={trimEnd} onChange={e=>setTrimEnd(+e.target.value)}/>{trimEnd}%</label><label>Speed <select value={speed} onChange={e=>setSpeed(+e.target.value)}><option>0.25</option><option>0.5</option><option>1</option><option>1.5</option><option>2</option><option>4</option></select>x</label></div>
      ...
      <div className="action-row"><button onClick={()=>{if(videoRef.current)videoRef.current.playbackRate=speed;preview()}}>▶ Preview Edit</button><button onClick={()=>onStatus('AI scene detection queued')}</button><button onClick={()=>onStatus('Auto captions queued for Bengali + English')}</button><button onClick={()=>onStatus('Video export will use the FFmpeg render pipeline')}>Export MP4 / WebM</button></div>
    </>}
