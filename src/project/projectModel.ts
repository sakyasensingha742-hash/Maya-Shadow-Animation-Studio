export type LayerKind = 'scene' | 'character' | 'rig' | 'background' | 'audio' | 'video' | 'drawing';
...
export function createDefaultScene(): Scene {
  return {
    id: makeId('scene'), name: 'Scene 01', width: 1920, height: 1080, fps: 24, duration: 240,
    tracks: [
...
export function createDefaultProject(): StudioProject {
  ...
  return {version: 1, name: 'Untitled Project', activeSceneId: scene.id, scenes: [scene], assets: [], createdAt: now, updatedAt: now};
}

export function addKeyframe(scene: Scene, trackId: string, frame: number, property: string, value: KeyframeValue): Scene {
  return {...scene, tracks: scene.tracks.map(t => t.id !== trackId ? t : {...t, keyframes: [...t.keyframes.filter(k => !(k.frame === frame && k.property === property)), {id: makeId('key'), frame, property, value, easing: 'linear'}].sort((a,b)=>a.frame-b.frame)})};
}
